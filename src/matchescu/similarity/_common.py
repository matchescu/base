from abc import abstractmethod, ABCMeta
from enum import Enum
from typing import Any


class Similarity(metaclass=ABCMeta):
    @abstractmethod
    def _compute_similarity(self, a: Any, b: Any) -> float:
        pass

    def __call__(self, a: Any, b: Any) -> float:
        if a is None and b is None:
            return 1
        elif a is None or b is None:
            return 0
        else:
            return self._compute_similarity(a, b)


class MatchResult(Enum):
    """Lists the possible results of a similarity function."""
    NonMatch = 0
    Match = 1
    NoComparisonData = 2

    def __repr__(self):
        return f"{self.value} - {self.name.lower()}"

    def __str__(self):
        return str(self.value)


class ValueSimilarityMatch:
    def __init__(self, similarity: Similarity, similarity_threshold: float = 0.9) -> None:
        self._threshold = similarity_threshold
        self._compute_similarity = similarity

    def __call__(self, a: Any, b: Any) -> MatchResult:
        if a is None and b is None:
            return MatchResult.NoComparisonData

        sim = self._compute_similarity(a, b)
        if sim < self._threshold:
            return MatchResult.NonMatch
        return MatchResult.Match
